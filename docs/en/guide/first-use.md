...
This guide first introduces the overall capabilities of 破晓 Daybreak, then walks through a complete CTF task from initial setup to automated solving. The walkthrough explains the main system modules and their intended usage.
...
When the system starts, it adds the local machine to the host management list by default, so 破晓 Daybreak can run without a remote host. In most scenarios, however, it is strongly recommended to configure a remote host and run sandbox containers remotely for security isolation and source-tracing resistance.
...
wget https://raw.githubusercontent.com/Linyisheng1/daybreak/refs/heads/main/sandbox/init_host.sh && chmod +x init_host.sh
```
...
破晓 Daybreak provides a web terminal that can connect directly to the remote host over SSH. Follow the instructions in [Quick Start](./quick-start#build-the-sandbox) to build the sandbox image.
