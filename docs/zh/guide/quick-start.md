...
破晓 Daybreak 的核心理念是：尽力做到极致的简洁，因此 破晓 Daybreak 的部署也将尽力简化步骤。

> :warning: 迭代说明
> 
> 目前，破晓 Daybreak 正处于快速迭代阶段，可能出现大幅度的结构、契约变动。为了保证代码质量，暂不考虑向前兼容。在生产环境使用时，建议做好合理的版本控制，历史数据可能需要额外的迁移措施。

## 开始之前

### 基础配置

破晓 Daybreak 的运行只需要以下核心内容和基础设施：

| 项目 | 说明 |
| --- | --- |
| `.daybreak/config.json` | 系统的运行时配置 |
| `.daybreak/agents/*` | 智能体的配置信息 |
| `Sandbox` | 定制的沙盒环境 |
| `Docker` | 沙盒环境运行时 |
| `PostgreSQL` | 数据持久化存储 |
...
破晓 Daybreak 的能力与沙盒环境深度绑定，因此需要构建对应的沙盒镜像：
...
编辑 `.daybreak/config.json` 中的系统运行时配置，主要修改以下内容：
...
一切准备就绪后，使用以下命令一键启动 破晓 Daybreak：
...
创建 `daybreak 数据库，数据库名称要与 config.json` 中填写的保持一致。

### 启动项目

创建 `.daybreak/config.json`，根据 `.daybreak/config.json.example` 中的示例填写相关信息。
...
根据 [首次使用](./first-use) 中的说明，正式开启 破晓 Daybreak 的工作！
