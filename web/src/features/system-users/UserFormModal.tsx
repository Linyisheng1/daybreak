...
export function UserFormModal({ open, mode, user, saving, onCancel, onSubmit }: UserFormModalProps) {
  ...
  return (
    <ResourceModal
      open={open}
      title={mode === "create" ? UI_TEXT.createUser : UI_TEXT.editUser}
      saving={saving}
      submitLabel={mode === "create" ? UI_TEXT.create : UI_TEXT.save}
      onCancel={onCancel}
      onSubmit={submit}
    >
      <label>
      ...
      <label>
        ...
        <Input mode="password" prefix={<KeyRound size={16} />} value={values.password} maxLength={128}
          ...
          placeholder={mode === "create" ? UI_TEXT.passwordPlaceholder : UI_TEXT.leaveBlankToKeep}
