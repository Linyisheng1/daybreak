...
export function PlaygroundSandboxCreateModal({ open, onCancel, onCreated }: PlaygroundSandboxCreateModalProps) {
  ...
  return (
    <ResourceModal
      open={open}
      title={UI_TEXT.createSandboxContainer}
      saving={saving}
      submitLabel={UI_TEXT.create}
    ...
    >
      <label>
        <span>{UI_TEXT.host}</span>
        <Select
          prefix={<Server size={16} />}
          value={hostId}
          loading={loading}
          disabled={loading || hosts.length === 0}
          placeholder={loading ? UI_TEXT.loadingHosts : UI_TEXT.selectManagedHost}
          emptyContent={loading ? <Spin size="small" /> : UI_TEXT.noHosts}
      ...
      <label>
        <span>{UI_TEXT.image}</span>
        <Select
          prefix={<Boxes size={16} />}
          value={imageId}
          loading={loading}
          disabled={loading || images.length === 0}
          placeholder={loading ? UI_TEXT.loadingImages : UI_TEXT.selectSandboxImage}
          emptyContent={loading ? <Spin size="small" /> : UI_TEXT.noImages}
      ...
      <label>
        <span>{UI_TEXT.egressMode}</span>
        <Select
          prefix={<Route size={16} />}
          value={egressMode}
          optionList={[
            { label: UI_TEXT.direct, value: SANDBOX_CONTAINER_EGRESS_MODE.DIRECT },
            { label: UI_TEXT.tor, value: SANDBOX_CONTAINER_EGRESS_MODE.TOR, disabled: !selectedImage?.supports_tor },
          ]}
          onChange={(value) => {
