...
export function ContainerFileManager({ containerId }: Props) {
  ...
  const handleCopy = useCallback(() => {
    ...
    Toast.success(`${selectedPaths.size} ${UI_TEXT.itemsCopied}`);
  }, [selectedPaths, path]);

  const handleCut = useCallback(() => {
    if (selectedPaths.size === 0) return;
    setClipboard({ action: "cut", paths: Array.from(selectedPaths), sourceDir: path });
    Toast.success(`${selectedPaths.size} ${UI_TEXT.itemsMoved}`);
  }, [selectedPaths, path]);
  ...
  const handleDelete = useCallback(async () => {
    ...
    await runFileAction(async () => {
      ...
      Toast.success(`${selectedPaths.size} ${UI_TEXT.itemsDeleted}`);
      await loadFiles(path);
    });
  ...
  const handleCreateConfirm = useCallback(async (name: string) => {
    ...
    await runFileAction(async () => {
      ...
      Toast.success(createType === "dir" ? UI_TEXT.directoryCreated : UI_TEXT.fileCreated);
      await loadFiles(path);
    });
      setCreateType(null);
  ...
  return (
    <div className="file-manager-body">
      <div className="file-manager-toolbar">
        ...
        <Button icon={<ArrowUp size={15} />} theme="borderless" type="tertiary" disabled={path === "/" || toolbarDisabled} onClick={goUp} aria-label="上级" />
        <Button icon={<RefreshCw size={15} />} theme="borderless" type="tertiary" disabled={toolbarDisabled} onClick={refresh} aria-label={UI_TEXT.refresh} />
        <span className="file-manager-separator" />
        <Button icon={<FilePlus size={15} />} theme="borderless" type="tertiary" disabled={toolbarDisabled || createType !== null} onClick={() => startCreate("file")} aria-label={UI_TEXT.newFile} />
        <Button icon={<FolderPlus size={15} />} theme="borderless" type="tertiary" disabled={toolbarDisabled || createType !== null} onClick={() => startCreate("dir")} aria-label={UI_TEXT.newFolder} />
        <Tooltip content={UI_TEXT.upload}>
        ...
        <Tooltip content={UI_TEXT.download}>
        ...
        <Tooltip content={canPaste ? UI_TEXT.pasteItems(clipboard?.paths.length ?? 0) : UI_TEXT.nothingToPaste}>
        ...
        <Popconfirm title={UI_TEXT.deleteSelectedItems} content={UI_TEXT.deleteSelectedConfirm(selectedPaths.size)} okType="danger" cancelText={UI_TEXT.cancel} onConfirm={() => void handleDelete()}>
      ...
      {viewMode === "list" ? (
        <div className="file-manager-list">
          ...
          {files.length === 0 && !createType ? (
            <div className="file-manager-empty">{loading ? UI_TEXT.loading : UI_TEXT.directoryEmpty}</div>
          ) : (
            files.map((file) => (
              <FileListRow
                key={file.path}
...
function FileListHeader() {
  return (
    <div className="file-manager-list-row file-manager-list-head" style={FILE_LIST_GRID_STYLE}>
      <div>{UI_TEXT.name}</div>
      <div>{UI_TEXT.size}</div>
      <div>{UI_TEXT.modified}</div>
      <div>{UI_TEXT.permissions}</div>
    </div>
  );
}
...
function InlineCreateInput({ type, onConfirm, onCancel, cancelOnBlur = false }: {
  ...
  return (
    <input
      ...
      placeholder={type === "dir" ? UI_TEXT.newFolder : UI_TEXT.newFile}
    />
  );
}
