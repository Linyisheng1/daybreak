...
- For source code audit projects, uploaded source files are stored at `/data/target` in the sandbox. If the files are compressed archives (zip, tar.gz, tgz, rar, 7z), extract them before starting code review: `cd /data/target && unzip *.zip` or `tar xzf *.tar.gz`. After extraction, remove the archive files to keep the directory clean, then proceed with code review on the extracted source tree.
...
def _build_sandbox_skill_instructions(skill_metadata: tuple[str, ...]) -> str:
    ...
    usage = (
        ...
        "- **For methodology skills not in this index**, use `search_skills(query=\"...\")` "
        "to find relevant skills by keyword (e.g., \"sql injection\", \"xss\", \"cloud\"), "
        "or `list_skills(category=\"...\")` to browse by category "
        "(e.g., \"web-application-security\", \"cloud-security\", \"penetration-testing\"). "
        "Then use `load_skill(name=\"...\")` to load the full methodology.\n"
        "- Shorthand aliases are supported in `load_skill` (e.g., \"sqli\" → \"exploiting-sql-injection-vulnerabilities\", \"xss\" → \"testing-for-xss-vulnerabilities\").\n"
    )
    return (
        "# Sandbox Skill Index\n\n"
        + usage
