...
async def compact_if_needed(
    *,
    ...
    if projected_tokens < trigger_tokens:
        logger.debug(
            "agent context compaction skipped: below threshold session=%s viewer=%s tokens=%d trigger=%d window=%d",
            scope.session_id,
...
def resolve_context_window(agent_config: AgentConfig) -> int:
    raw = agent_config.context_window
    cap = get_config().session_health.context_window_sanity_cap
    if raw > cap:
        logger.warning(
            "agent context_window=%d exceeds sanity cap=%d (agent=%s); capping to %d to ensure compaction triggers",
            raw, cap, agent_config.code, cap,
        )
        return cap
    return raw
