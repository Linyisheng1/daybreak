...
async def query_available_sandbox_containers_handler(
    ...
    if sandbox_containers.total == 0 and not include_non_running and work_project_id is None:
        # No available containers — try auto-starting user's stopped/created containers
        try:
            await _auto_start_user_containers(user_id, user_role)
            sandbox_containers = await query_available_sandbox_containers(
                page=page, size=size, keyword=keyword,
                user_id=user_id, user_role=user_role,
                work_project_id=work_project_id, include_non_running=include_non_running,
            )
        except Exception:
            logger.warning("auto-start containers failed", exc_info=True)
...
async def _auto_start_user_containers(user_id: int, user_role: SystemUserRole) -> None:
    """Find STOPPED/CREATED containers owned by user and start them."""
    from sqlmodel import select, or_
    from database import get_async_session
    from model.sandbox.containers import SandboxContainer
    from model.work_project.projects import WorkProject, WorkProjectSandboxContainer, WorkProjectStatus
    from service.sandbox.lifecycle import start_sandbox_container
    ...
    stop_start_states = {SandboxContainerStatus.STOPPED, SandboxContainerStatus.CREATED}
    # Admin sees all containers, regular users only their own
    if user_role == SystemUserRole.ADMIN:
        base_filter = SandboxContainer.status.in_(stop_start_states)
    else:
        base_filter = SandboxContainer.status.in_(stop_start_states) & (SandboxContainer.owner_id == user_id)

    # Exclude containers bound to active (WORKING) projects
    bound_to_active = exists(
        select(WorkProjectSandboxContainer)
        .join(WorkProject, WorkProject.id == WorkProjectSandboxContainer.project_id)
        .where(
            WorkProjectSandboxContainer.sandbox_container_id == SandboxContainer.id,
            WorkProject.status == WorkProjectStatus.WORKING,
        )
    )
    ...
    async with get_async_session() as session:
        containers = (
            await session.exec(
                select(SandboxContainer).where(base_filter).where(~bound_to_active)
            )
        ).all()

    for container in containers:
        try:
            result = await start_sandbox_container(container.id)
            if result.succeeded:
                logger.info("auto-started sandbox container: %s", container.id)
            else:
                logger.warning("auto-start failed for container %s: %s", container.id, result.message)
        except Exception:
            logger.exception("auto-start error for container %s", container.id)
