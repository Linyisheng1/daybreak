...
_MAX_SANDBOX_SKILLS = 96


# Per-agent skill category priority — skills from these categories appear
# first in the metadata index for each specialist, ensuring the most relevant
# methodologies are immediately visible without needing search_skills.
AGENT_SKILL_CATEGORIES: dict[str, list[str]] = {
    # CPE (Fr4nk): Chief Penetration Engineer
    "cpe": [
        "penetration-testing", "web-application-security", "network-security",
        "red-teaming", "offensive-security", "api-security", "wireless-security",
    ],
    # CIE (L1ly): Chief Intelligence Engineer
    "cie": [
        "reconnaissance", "threat-intelligence", "threat-detection",
        "web-application-security", "phishing-defense", "social-engineering-defense",
    ],
    # CAE (V3ra): Chief Audit Engineer
    "cae": [
        "application-security", "vulnerability-management", "devsecops",
        "supply-chain-security", "web-application-security",
    ],
    # CRE (J4m3): Chief Reverse Engineer
    "cre": [
        "malware-analysis", "digital-forensics", "firmware-security",
        "mobile-security", "endpoint-security",
    ],
    # CCE (Nu1L): Chief Cryptography Engineer
    "cce": [
        "cryptography", "identity-access-management", "zero-trust-architecture",
        "data-protection", "cloud-security", "container-security",
    ],
}
...
async def build_runtime_context(
    ...
    if binding is not None:
        ...
        sandbox_skill_metadata = await _load_sandbox_skill_metadata(binding.id, agent_code)
...
async def _load_sandbox_skill_metadata(container_id: int, agent_code: str = "") -> tuple[str, ...]:
    try:
        result = await execute_sandbox_container_command(
            id=container_id,
            command=_build_skill_metadata_command(agent_code),
            timeout_seconds=30,
        )
...
def _build_skill_metadata_command(agent_code: str = "") -> str:
    skills_dir = shlex.quote(SANDBOX_SKILLS_DIR)
    max_skills = _MAX_SANDBOX_SKILLS

    # Determine priority categories for this agent
    priority_categories = AGENT_SKILL_CATEGORIES.get(agent_code, [])

    if priority_categories:
        # Build a two-pass command:
        # Pass 1: Skills whose front matter subdomain matches a priority category
        # Pass 2: Fill remaining slots with any skills not yet listed
        cat_pattern = "|".join(priority_categories)
        return f"""
if [ -d {skills_dir} ]; then
  PRIORITY_CATS="{cat_pattern}"
  COUNT=0
  LISTED=""
  # Pass 1: Priority-category skills
  find {skills_dir} -mindepth 2 -maxdepth 2 -name SKILL.md -type f | sort | while IFS= read -r skill_file; do
    if grep -qE "subdomain: ($PRIORITY_CATS)" "$skill_file" 2>/dev/null; then
      skill_name=$(basename "$(dirname "$skill_file")")
      printf '===SKILL:%s===\\n' "$skill_name"
      awk 'NR == 1 && $0 == "---" {{ print; in_fm = 1; next }} in_fm {{ print; if ($0 == "---") exit }}' "$skill_file"
    fi
  done
  # Pass 2: Fill remaining slots with any skills (tool-reference skills first, then alphabetical)
  find {skills_dir} -mindepth 2 -maxdepth 2 -name SKILL.md -type f | sort | while IFS= read -r skill_file; do
    skill_name=$(basename "$(dirname "$skill_file")")
    if grep -qE "subdomain: ($PRIORITY_CATS)" "$skill_file" 2>/dev/null; then
      continue
    ...
    printf '===SKILL:%s===\\n' "$skill_name"
    awk 'NR == 1 && $0 == "---" {{ print; in_fm = 1; next }} in_fm {{ print; if ($0 == "---") exit }}' "$skill_file"
  done | head -n {max_skills * 20}
fi
""".strip()
    else:
        # No priority categories (cso or unknown agent): use alphabetical with cap
        return f"""
if [ -d {skills_dir} ]; then
  find {skills_dir} -mindepth 2 -maxdepth 2 -name SKILL.md -type f | sort | head -n {max_skills} | while IFS= read -r skill_file; do
    skill_name=$(basename "$(dirname "$skill_file")")
    printf '===SKILL:%s===\\n' "$skill_name"
    awk 'NR == 1 && $0 == "---" {{ print; in_fm = 1; next }} in_fm {{ print; if ($0 == "---") exit }}' "$skill_file"
  done
...
def _parse_skill_metadata_output(output: str) -> list[str]:
    ...
    for raw_line in output.splitlines():
        if raw_line.startswith("===SKILL:") and raw_line.endswith("==="):
            if len(blocks) >= _MAX_SANDBOX_SKILLS:
                break
