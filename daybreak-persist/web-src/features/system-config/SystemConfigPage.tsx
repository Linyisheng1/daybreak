...
const RUNTIME_FIELDS: ConfigField<AgentRuntimeConfig>[] = [
  { kind: "number", key: "main_max_turns", label: "主最大轮次", min: 1 },
  { kind: "number", key: "subordinate_max_turns", label: "子代理最大轮次", min: 1 },
  { kind: "number", key: "model_stream_idle_timeout_seconds", label: "流式空闲超时", min: 30 },
  { kind: "toggle", key: "context_compression_enabled", label: "上下文压缩" },
  { kind: "number", key: "context_compression_trigger_ratio", label: "触发比率", min: 0.01, max: 0.99, step: 0.01 },
  { kind: "number", key: "context_compression_hard_stop_ratio", label: "硬停止比率", min: 0.01, max: 0.99, step: 0.01 },
  { kind: "number", key: "context_compression_target_ratio", label: "目标比率", min: 0.01, max: 0.99, step: 0.01 },
  { kind: "number", key: "context_budget_model_call_ratio", label: "模型调用预算", min: 0.01, max: 0.99, step: 0.01 },
  { kind: "number", key: "context_compression_preserve_recent_ratio", label: "保留近期比率", min: 0.01, max: 0.99, step: 0.01 },
  { kind: "number", key: "context_compression_preserve_recent_items", label: "保留近期项数", min: 1 },
  { kind: "number", key: "context_compression_min_items", label: "最小项数", min: 1 },
...
const POOL_FIELDS: ConfigField<AgentPoolConfig>[] = [
  { kind: "number", key: "max_size", label: "最大大小", min: 1 },
  { kind: "number", key: "ttl_seconds", label: "TTL 秒数", min: 0 },
  { kind: "number", key: "sweep_interval_seconds", label: "清理间隔秒数", min: 1 },
];
...
const AGENT_TEXT_FIELDS: AgentTextField[] = [
  ...
  { key: "model", label: "模型" },
  { key: "api_key", label: "API 密钥", password: true },
];
...
export function SystemConfigPage() {
  ...
  const metrics = useMemo(() => {
    ...
    return [
      ...
      { label: "主轮次", value: values?.agent_runtime.main_max_turns ?? "-" },
      { label: "压缩", value: values?.agent_runtime.context_compression_enabled ? "已启用" : "已禁用" },
    ];
  }, [values]);
      ...
      <Spin spinning={loading} wrapperClassName="system-config-spin">
        {values ? (
          <div className="system-config-layout">
            ...
            <ConfigPanel icon={<Bot size={18} />} title="智能体">
