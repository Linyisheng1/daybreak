...
export function HostsPage() {
  ...
  useAdminResourceHeader({
    createLabel: UI_TEXT.createHost,
    refreshLabel: UI_TEXT.refreshHosts,
    loading,
  ...
  const columns: ResourceColumn<ManagedHost>[] = [
    {
      key: "host", header: UI_TEXT.host, width: "minmax(0, 0.7fr)",
      render: (host) => (
        <ResourceIdentity icon={<Server size={18} />} title={host.ip_address} detail={`SSH ${host.ssh_port}`} />
      ),
    },
    {
      key: "account", header: UI_TEXT.account, width: "minmax(0, 0.5fr)",
      render: (host) => <OwnerCell>{host.host_account}</OwnerCell>,
    },
    {
      key: "password", header: UI_TEXT.password, width: "minmax(0, 0.6fr)",
      render: (host) => (
    ...
    {
      key: "docker", header: UI_TEXT.dockerPorts, width: "110px",
      render: (host) => host.docker_management_port,
    },
    {
      key: "tls", header: UI_TEXT.type, width: "90px",
      render: (host) => (
        <Tag color={host.docker_tls_enabled ? "green" : "grey"}>
          {host.docker_tls_enabled ? UI_TEXT.tls : UI_TEXT.plain}
        </Tag>
      ),
    },
    { key: "updated", header: UI_TEXT.updated, width: "minmax(0, 0.7fr)", render: (host) => formatDateTime(host.updated_at) },
    {
      key: "actions", header: UI_TEXT.actions, width: "140px",
      render: (host) => (
        <RowActions>
          <Button icon={<SquareTerminal size={15} />} theme="borderless" type="tertiary"
            aria-label={`连接 Shell ${host.ip_address}`} onClick={() => openHostShell(host)}
          />
          <Button icon={<Boxes size={15} />} theme="borderless" type="tertiary"
            aria-label={`管理镜像 ${host.ip_address}`} onClick={() => setImageModalHost(host)}
          />
          <Button icon={<Pencil size={15} />} theme="borderless" type="tertiary"
            aria-label={`${UI_TEXT.edit} ${host.ip_address}`} onClick={() => setModal({ mode: "edit", host })}
          />
          <Popconfirm title={UI_TEXT.deleteHostTitle} content={UI_TEXT.deleteConfirm(host.ip_address)} okType="danger" cancelText={UI_TEXT.cancel} onConfirm={() => void deleteHost(host)}>
            <Button icon={<Trash2 size={15} />} theme="borderless" type="danger"
              loading={deletingHostId === host.id} aria-label={`${UI_TEXT.delete} ${host.ip_address}`}
            />
          </Popconfirm>
        </RowActions>
      ),
  ...
  return (
    <>
      <ResourcePageShell
        searchPlaceholder={UI_TEXT.searchIpAccount}
        keyword={keyword}
        loading={loading}
        metrics={[
          { label: UI_TEXT.total, value: total },
          { label: UI_TEXT.ssh, value: summary.ssh },
          { label: UI_TEXT.dockerPorts, value: summary.docker },
        ]}
        empty={hosts.length === 0}
        emptyIcon={<Server size={42} />}
        emptyTitle={UI_TEXT.noHostsFound}
        page={page}
...
function HostImagesModal({ host, onClose }: { host: ManagedHost | null; onClose: () => void }) {
  ...
  return (
    <Modal
      title={host ? UI_TEXT.imagesOnHost(host.ip_address) : UI_TEXT.hostImages}
    ...
    >
      <div className="host-images-toolbar">
        <Select
          ...
          placeholder={UI_TEXT.selectImagesToPull}
      ...
      <Table
        ...
        columns={[
          { title: UI_TEXT.image, dataIndex: "image_name" },
          { title: UI_TEXT.hash, dataIndex: "image_hash", width: 120, render: (value) => String(value || "").slice(0, 12) || "-" },
          { title: UI_TEXT.size, dataIndex: "image_size", width: 100, render: (value) => formatBytes(Number(value || 0)) },
          {
            title: "", dataIndex: "image_id", width: 50,
            render: (_value, record) => (
              <Popconfirm title={UI_TEXT.removeImage} content={UI_TEXT.removeImageConfirm((record as ManagedHostImage).image_name || "this image")} okType="danger" cancelText={UI_TEXT.cancel} onConfirm={() => void removeImage(record as ManagedHostImage)}>
                <Button icon={<Trash2 size={14} />} theme="borderless" type="danger" size="small"
                  loading={removingId === (record as ManagedHostImage).image_id}
                  aria-label={UI_TEXT.removeImage}
                />
              </Popconfirm>
            ),
          },
