    ...
    <div className={cx("execution-row", `execution-row-${status.tone}`)}>
      ...
      {open ? (
        <div ref={detailRef} className="execution-row-detail">
          <JsonExecutionSection label={UI_TEXT.arguments} value={item.arguments} />
          ...
          {item.resolved ? (
            ...
            <ExecutionSection label={UI_TEXT.output} body={UI_TEXT.pending} />
          )}
        </div>
      ) : null}
    </div>
  ...
  return (
    <div className={cx("execution-row execution-row-subagent", `execution-row-subagent-${item.status}`, selected && "execution-row-selected")}>
      <div className="execution-row-head execution-row-head-static">
        <ExecutionName name={item.agentCode || UI_TEXT.subagent} />
    ...
    <div className={cx("nested-panel", live && "nested-panel-live", selected && "nested-panel-selected")}>
      <div className="nested-panel-head">
        ...
        <span className="nested-panel-title">
          {UI_TEXT.subagent}{task?.agentCode ? ` - ${task.agentCode}` : nested.agentName ? ` - ${nested.agentName}` : ""}
        </span>
...
function ToolOutputSection({ output, tone }: { output: string; tone?: "error" }) {
  ...
  if (!parsed.ok) {
    return <ExecutionSection label={UI_TEXT.output} body={output || UI_TEXT.empty} tone={tone} />;
  }
  return <JsonExecutionSection label={UI_TEXT.output} value={parsed.value} tone={tone} />;
}

function toolExecutionStatus(item: ToolExecutionItem): { label: string; color: "red" | "green" | "amber"; tone: "error" | "ok" | "running" } {
  if (item.resolved && item.isError) return { label: UI_TEXT.failed, color: "red", tone: "error" };
  ...
  if (!item.resolved || item.subagentTask?.status === "running") return { label: UI_TEXT.running, color: "amber", tone: "running" };
  return { label: UI_TEXT.done, color: "green", tone: "ok" };
}
