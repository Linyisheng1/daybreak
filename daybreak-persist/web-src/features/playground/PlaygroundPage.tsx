...
const STATUS_LABEL: Record<string, string> = {
  open: UI_TEXT.live,
  connecting: UI_TEXT.connecting,
  closed: UI_TEXT.disconnected,
  idle: UI_TEXT.idle,
};
...
export function PlaygroundPage() {
  ...
  const sandboxManageUnavailableReason = sandboxAccessUnavailableReason ? "无权操作此沙箱" : null;
  ...
  const headerNode = useMemo(() => (
    <>
      ...
      <div className="sandbox-container-actions" aria-label="Selected sandbox actions">
        <SandboxActionButton
          ariaLabel={UI_TEXT.createSandboxContainer}
          disabled={Boolean(activeProjectId)}
          icon={<Box size={15} />}
          tooltip={activeProjectId ? "项目会话使用项目绑定的沙箱" : UI_TEXT.createSandboxContainer}
        ...
        <SandboxActionButton
          ...
          tooltip={sandboxManageUnavailableReason ?? (canStartSelectedSandbox ? `启动 ${selectedSandboxName}` : "选择一个已创建或已停止的沙箱")}
        ...
        <SandboxActionButton
          ...
          tooltip={sandboxManageUnavailableReason ?? (canStopSelectedSandbox ? `停止 ${selectedSandboxName}` : "选择一个运行中的沙箱")}
        ...
        <SandboxActionButton
          ...
          tooltip={sandboxManageUnavailableReason ?? (canPauseSelectedSandbox ? `暂停 ${selectedSandboxName}` : "选择一个运行中的沙箱")}
        ...
        <SandboxActionButton
          ...
          tooltip={sandboxManageUnavailableReason ?? (canResumeSelectedSandbox ? `恢复 ${selectedSandboxName}` : "选择一个已暂停的沙箱")}
          onClick={() => void runSandboxMutation("resume", selectedSandboxContainer)}
        />
        <Popconfirm
          title={UI_TEXT.deleteContainer}
          content={selectedSandboxContainer ? `删除 ${selectedSandboxContainer.container_name}?` : "请先选择一个沙箱"}
          okType="danger"
        ...
        >
          <span>
            <SandboxActionButton
              ...
              tooltip={sandboxManageUnavailableReason ?? (selectedSandboxContainer ? `删除 ${selectedSandboxName}` : "请先选择一个沙箱")}
              onClick={() => undefined}
            />
          </span>
        </Popconfirm>
        <SandboxActionButton
          ariaLabel={`${UI_TEXT.openTerminal} ${selectedSandboxName}`}
          disabled={Boolean(shellUnavailableReason)}
          icon={<SquareTerminal size={15} />}
          tooltip={shellUnavailableReason ?? `${UI_TEXT.openTerminal} ${selectedSandboxName}`}
          onClick={openSelectedShell}
        />
        <SandboxActionButton
          ariaLabel={`${UI_TEXT.openScreen} ${selectedSandboxName}`}
          disabled={Boolean(screenUnavailableReason)}
          icon={<Monitor size={15} />}
          tooltip={screenUnavailableReason ?? `${UI_TEXT.openScreen} ${selectedSandboxName}`}
          onClick={openSelectedNoVNC}
        />
        <SandboxActionButton
          ariaLabel={`${UI_TEXT.browseFiles} ${selectedSandboxName}`}
          disabled={Boolean(shellUnavailableReason)}
          icon={<FolderOpen size={15} />}
          tooltip={shellUnavailableReason ?? `${UI_TEXT.browseFiles} ${selectedSandboxName}`}
          onClick={openSelectedFileManager}
        />
        {activeProjectId ? (
          <SandboxActionButton
            ariaLabel={UI_TEXT.projectInfo}
            disabled={false}
            icon={<FolderKanban size={15} />}
            tooltip={UI_TEXT.projectInfo}
            onClick={openProjectRecords}
          />
        ) : null}
        <SandboxActionButton
          ariaLabel={UI_TEXT.openSubagentPanel}
          disabled={subagentTabs.length === 0}
          icon={<PanelRightOpen size={15} />}
          tooltip={subagentTabs.length > 0 ? UI_TEXT.openSubagentPanel : UI_TEXT.noSubagentMessages}
      ...
      <span className={cx("stream-status", `stream-status-${status}`)}>
        <Activity size={14} />
        <span>{STATUS_LABEL[status] ?? UI_TEXT.idle}</span>
      </span>
    </>
  ), [
    activeProjectId,
...
function getSandboxActionUnavailableReason(
  ...
  if (!container) return "请先选择一个沙箱";
  if (container.status !== SANDBOX_CONTAINER_STATUS.RUNNING) return "所选沙箱未运行";
  if (options.requiresControlProxy && container.control_proxy_host_port <= 0) return "所选沙箱控制端口未就绪";
  if (options.requiresNoVNC && !canOpenContainerNoVNC(container)) return "所选沙箱无 noVNC 屏幕";
  return null;
}
