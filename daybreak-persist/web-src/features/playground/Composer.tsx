...
export function Composer({
  ...
  const addImageFiles = useCallback(async (files: File[]) => {
    ...
    if (available === 0) {
      Toast.warning(UI_TEXT.atMostImagesAllowed(MAX_IMAGES));
      return;
    }
    ...
    for (const file of imageFiles.slice(0, available)) {
      if (file.size > MAX_IMAGE_BYTES) {
        Toast.warning(UI_TEXT.imageExceedsSize(file.name));
        continue;
      }
      if (currentBytes + nextBytes + file.size > MAX_TOTAL_IMAGE_BYTES) {
        Toast.warning(UI_TEXT.totalImageExceedsSize);
        continue;
      }
      try {
        ...
        Toast.error(UI_TEXT.failedToReadFile(file.name));
      }
    }
    if (next.length) {
  ...
  const agentSwitchDisabledReason = UI_TEXT.switchAgentSubagentWarning;
  ...
  const action = streaming
    ? { icon: <Square size={16} />, type: "danger" as const, title: UI_TEXT.stop, onClick: onInterrupt, disabled: false }
    : {
        icon: <Send size={16} />,
      ...
      <div className="composer-input">
        ...
        <div className="composer-panel">
          ...
          <TextArea
            ...
            placeholder={
              disabled
                ? UI_TEXT.loadingHistory
                : streaming
                  ? UI_TEXT.streamingPlaceholder
                  : UI_TEXT.sendPlaceholder
            }
          />
          <input
            ref={fileInputRef}
          ...
          <div className="composer-footer">
            <button
              ...
              onClick={toggleChip}
              disabled={agentSwitchDisabled}
              aria-label={activeAgent ? UI_TEXT.speakingTo(activeAgent.name) : UI_TEXT.pickAgent}
              title={agentSwitchDisabled ? agentSwitchDisabledReason : activeAgent ? UI_TEXT.clickToSwitchAgent : UI_TEXT.pickAgent}
            >
              <AtSign size={14} />
              <span>{activeAgent?.name || UI_TEXT.agent}</span>
            </button>
            <div className="composer-actions">
              <Button
                ...
                aria-label={UI_TEXT.attachImage}
                title={UI_TEXT.attachImage}
              />
              <Button
                className="composer-action-button"
                icon={action.icon}
                ...
                aria-label={streaming ? UI_TEXT.interruptStreaming : UI_TEXT.sendMessage}
                title={action.title}
              />
              <Button
                className="composer-action-button"
                ...
                aria-label={UI_TEXT.cancelAllSubagentTasks}
                title={canCancelAll ? UI_TEXT.cancelAllSubagentTasks : UI_TEXT.noRunningSubagentTasks}
              />
            </div>
          </div>
        </div>
