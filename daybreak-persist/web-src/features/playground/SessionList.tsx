  ...
  return (
    <div className="session-list">
      ...
      <Modal
        ...
        title={UI_TEXT.editSessionTitle}
        okText={UI_TEXT.save}
...
function ChatSessionRow({ session, active, onSelect, onDelete, onRename }: ChatSessionRowProps) {
  return (
    <SessionRow
      active={active}
      deleteConfirm={{
        title: UI_TEXT.deleteChat,
        content: UI_TEXT.deleteChatConfirm,
        onConfirm: () => onDelete(session.session_id),
      }}
      icon={<MessageCircle size={14} />}
      session={session}
      titleFallback={UI_TEXT.untitledSession}
    ...
    <div className="session-project-group">
      ...
      {expanded ? (
        <div className="session-project-children">
          {state?.loading ? <div className="session-project-empty">{UI_TEXT.loadingSessions}</div> : null}
          {!state?.loading && (!state || state.items.length === 0) ? (
            ...
            >
              ...
              <span>{UI_TEXT.newProjectSession}</span>
            </button>
          ) : null}
...
function ProjectSessionRow({
  ...
  return (
    <SessionRow
      ...
      deleteConfirm={{
        title: UI_TEXT.deleteSession,
        content: UI_TEXT.deleteSessionConfirm,
        onConfirm: () => onDelete(projectId, session.session_id),
      }}
      icon={<Play size={13} />}
      session={session}
      titleFallback={UI_TEXT.projectSession}
